...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tracker_project_name",
        type=str,
        default="CapillaDiff-Label2Image-fine-tune",
        help=(
...
def encode_prompt(identifier, args, clip=None):
    """Get gene embedding generated by scGPT based on input identifier.

    Args:
        identifier (str): condition identifier
        args (argparse.Namespace): The parsed arguments.
        clip (tuple or None): If using CLIP, provide (clip_tokenizer, clip_text_encoder, device).

    Returns:
        prompt_embeds (torch.Tensor): gene embedding"""

    encoder = ConditionEncoder(args.naive_conditional, convert_to_boolean=args.convert_to_boolean, text_mode=args.text_mode, clip=clip)
...
def main():
    ...
    def collate_fn(examples, args, caption_column='caption', clip=None):
        ...
        prompt_embeds = [encode_prompt(
            example[caption_column], args=args, clip=clip) for example in examples]
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=True,
        collate_fn=lambda examples: collate_fn(examples, args, clip=clip),
