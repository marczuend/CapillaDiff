...
def get_condition_embedding(identifier, model_type, convert_to_boolean, text_mode=None, clip_path=None):
    """Get embedding generated by scGPT based on input sirna id.
    ...
    if model_type == 'conditional':
        ...
        if text_mode is None:
            ...
            raise NotImplementedError("Text mode is not implemented yet.")
            ...
            CLIP_MODEL_PATH = "/cluster/customapps/medinfmk/mazuend/CapillaDiff/models/clip-vit-large-patch14"
            ...
            # 3. Encode text
            inputs = tokenizer(condition_text, padding=True, truncation=True, return_tensors="pt").to(device)

            with torch.no_grad():
                outputs = text_encoder(**inputs)

            # 4. Get embeddings (pooled output)
            embedding = outputs.last_hidden_state.mean(dim=1)  # [batch_size, hidden_dim]
...
class ConditionEncoder:
    ...
    def __init__(self, model_type: str='conditional', convert_to_boolean: bool = True, text_mode: str=None):
    ...
    def get_condition_embedding(self, identifier):
        ...
        return get_condition_embedding(
            identifier, self.model_type, self.convert_to_boolean, self.text_mode)

class ConditionEncoderInference:

    def __init__(self, model_type, convert_to_boolean: bool = True, text_mode: str = None):
        self.model_type = model_type
        self.convert_to_boolean = convert_to_boolean
        self.text_mode = text_mode
    ...
    def get_embedding(self, identifier):
        ...
        """
        return get_condition_embedding(
            identifier, self.model_type, self.convert_to_boolean)
